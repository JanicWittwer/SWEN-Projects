...
# Kombinierte Version - neu per 05.12.2022
## Neue Funktionen, neuer Aufbau des Codes, etc.
...
def punkte(wuerfelraus):
    zwischenpunktestand = 0
    
    for wurf in range(1,7):
        if wuerfelraus.count(wurf) >= 3:
            if wurf == 1:
                zwischenpunktestand += 1000
            else:
                zwischenpunktestand += wurf *100
        elif wuerfelraus.count(wurf) >= 1:
            if wurf == 1:
                zwischenpunktestand += 100 * wuerfelraus.count(wurf)
            if wurf == 5:
                zwischenpunktestand += 50 * wuerfelraus.count(wurf)

    return zwischenpunktestand


#Hier erstellen wir die verschiedenen Spieler-Konten
class Spieler:
    def __init__(self, name):
        self.name = name
        self.punktestand_temp = 0
...
def spielerwechsel():
    self.punktestand_total = self.punktestand_temp + zwischenpunktestand
...
def main():
    ...
    #Wenn ich Punkte gemacht haben, dann können wir nun unsere Würfel auswählen. Das wiederholt sich nun, bis alle Würfel rausgenommen wurden.
    if hatpunkte(wuerfelliste) == True:
        zwischenstand = auswahl(wuerfelliste)
        print(f"Folgende Würfel hast du bereits herausgenommen " + str(zwischenstand))
        print(f"Die folgenden Würfen hast du noch zur Auswahl " + str(wuerfelliste))
